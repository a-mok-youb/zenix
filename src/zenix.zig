...
fn renderTemplate(template: []const u8, replacements: []const Replacement) ![]u8 {
    var result = template;

    for (replacements) |rep| {
        const replaced = try std.mem.replaceOwned(u8, std.heap.page_allocator, result, rep.key, rep.value);
...
fn rendercomponent(input: []u8) ![]u8 {
    var result = input;
    ...
    while (true) {
        ...
        const comp_path = try std.fmt.allocPrint(std.heap.page_allocator, "frontend/components/{s}.html", .{comp_name});
...
pub fn renderPageWithLayout( pagePath: []const u8, layoutPath: []const u8, title: []const u8, replacements: []const Replacement,) ![]u8 {
...
pub fn errorResponse(status: u16, message: []const u8) ![]u8 {
    const status_str = try std.fmt.allocPrint(std.heap.page_allocator, "{d}", .{status});
    defer std.heap.page_allocator.free(status_str);
    const body = try renderPageWithLayout("frontend/pages/error.html", "frontend/layouts/main_layout.html", status_str, &.{
